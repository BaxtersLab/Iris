...
impl LoggedTelemetryReceiver {
    pub fn new(inner: broadcast::Receiver<TelemetryEnvelope>) -> Self {
        ...
        Self { inner, last_sequence: None, id }
    }
    ...
    pub fn try_recv(&mut self) -> Result<TelemetryEnvelope, broadcast::error::TryRecvError> {
        match self.inner.try_recv() {
            ...
            Err(e) => {
                println!("LoggedTelemetryReceiver(id={}): try_recv error ts={} err={:?}", self.id, Utc::now(), e);
                Err(e)
            }
        }
    }
    ...
    pub async fn recv(&mut self) -> Result<TelemetryEnvelope, broadcast::error::RecvError> {
        println!("LoggedTelemetryReceiver(id={}): recv await ts={}", self.id, Utc::now());
        match self.inner.recv().await {
            ...
            Err(e) => {
                println!("LoggedTelemetryReceiver(id={}): recv error ts={} err={:?}", self.id, Utc::now(), e);
                Err(e)
            }
        }
    }
...
impl IpcServer {
    pub fn new(buffer_size: usize) -> (Self, IpcHandle, tokio::sync::broadcast::Sender<TelemetryEnvelope>) {
        ...
        (
            Self { cmd_rx, telemetry_tx: telemetry_tx.clone() },
            IpcHandle { cmd_tx, telemetry_rx, telemetry_tx: telemetry_tx.clone() },
            telemetry_tx.clone(),
        )
    }
    ...
    pub async fn run(mut self) {
        while let Some((cmd, resp_tx)) = self.cmd_rx.recv().await {
            match cmd {
                IpcCommand::Ping => {
                    let uptime = SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_millis() as u64).unwrap_or(0);
    ...
    pub fn emit_telemetry(&self, event: TelemetryEvent) {
        let envelope = TelemetryEnvelope { timestamp: chrono::Utc::now(), sequence: 0, event: event.clone() };
        println!("IpcServer: emit_telemetry event={:?} seq=0 ts={}", envelope.event, envelope.timestamp);
...
impl IpcHandle {
    pub async fn send_command(&self, cmd: IpcCommand) -> IrisResult<IpcResponse> {
        ...
        self.cmd_tx.send((cmd, resp_tx)).await.map_err(|e| iris_core::error::IrisError::Ipc(format!("send failed: {}", e)))?;
        let resp = resp_rx.await.map_err(|e| iris_core::error::IrisError::Ipc(format!("recv failed: {}", e)))?;
        Ok(resp)
    }
