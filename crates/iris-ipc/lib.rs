...
pub use server::{IpcServer, IpcHandle, LoggedTelemetryReceiver};
...
pub trait Dispatcher: Send + 'static {
	fn dispatch(&mut self, cmd: command::IpcCommand) -> Pin<Box<dyn Future<Output = response::IpcResponse> + Send>>;
}
