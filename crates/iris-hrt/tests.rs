...
async fn test_hrt_health_tick() {
    ...
    let (service, handle) = HrtService::new(HrtConfig { interval_ms: 10, ..HrtConfig::default() }, tx);
    let srv = task::spawn(service.run());
    handle.send(HrtCommand::Start).await.unwrap();
...
async fn test_hrt_force_check() {
    ...
    let got = rx.recv().await.unwrap();
    assert!(matches!(got.event, iris_ipc::telemetry::TelemetryEvent::HealthCheck { .. }));
    handle.send(HrtCommand::Shutdown).await.unwrap();
    srv.abort();
}
...
async fn test_hrt_set_interval() {
    ...
    let (service, handle) = HrtService::new(HrtConfig::default(), tx);
    let srv = task::spawn(service.run());
    handle.send(HrtCommand::SetInterval { interval_ms: 5 }).await.unwrap();
    handle.send(HrtCommand::Start).await.unwrap();
...
async fn test_hrt_usb_bandwidth_warning() {
    let (tx, mut rx) = tokio::sync::broadcast::channel(16);
    let (service, handle) = HrtService::new(HrtConfig { usb_bandwidth_threshold: 0.5, ..HrtConfig::default() }, tx);
    let srv = task::spawn(service.run());
    ...
    assert!(matches!(got.event, iris_ipc::telemetry::TelemetryEvent::HealthCheck { .. } | iris_ipc::telemetry::TelemetryEvent::UsbBandwidthWarning { .. }));
    handle.send(HrtCommand::Shutdown).await.unwrap();
    srv.abort();
}
